...
class XPINN:
    ...
    def callback(self, loss):
        ...
        if step % 250 == 0:
            ...
            print()
            np.savez(f'数据+结果/数据/w={omega}_MSE_LBFGS_hist.npz',
            self.MSE_LBFGS_hist1)
...
if __name__ == "__main__":
    ...
    np.savez(f"./数据+结果/数据/w={omega}_MSE_hist.npz",
                      MSE_hist1, w_ub1_hist1, np.array(w_f1_hist1))
    ...
    MSE_hist1 = data['arr_0']
    w_ub1_hist1 = data['arr_1']
    w_f1_hist1 = data['arr_2']
    load_file1 = f'./数据+结果/数据/w={omega}_MSE_LBFGS_hist.npz'
    data1 = np.load(load_file1)
    MSE_LBFGS_hist1 = data1['arr_0']
    MSE_adam_LBFGS_hist = np.hstack((MSE_hist1, MSE_LBFGS_hist1))
    actual_iterations = len(MSE_hist1) * 250
    actual_iterations1 = len(MSE_adam_LBFGS_hist) * 250
    ...
    np.savez(
        f'./数据+结果/数据/w={omega}_predict.npz',
    ...
    fig1 = plt.figure()
    plt.plot(range(1, actual_iterations1 + 1, 250), MSE_adam_LBFGS_hist, 'r-', linewidth=2.2, label='Loss')
    plt.xlabel('$\#$ iterations', fontsize=14)
    plt.ylabel('Loss', fontsize=14)
    plt.yscale('log')
    plt.legend(loc='upper right')
    plt.savefig(f'./数据+结果/结果/w={omega}_LossDeclineCurve.png', dpi=1000)
    plt.show()